  ...
  return (
    <div style={{ borderRadius: '5px', backgroundColor: isDarkMode ? '#333' : 'pink', color: isDarkMode ? '#fff' : '#000', padding: '20px', marginBottom: '20px', width: '400px' }}>
      ...
      <div style={{ display: 'flex', marginBottom: '10px' }}>
        <button onClick={handleUndo} style={{ borderRadius: '5px', backgroundColor: 'lightyellow', marginRight: '10px', padding: '10px' }}>
          <FaUndo />
        </button>
      </div>
      {showEmojiPicker && (
      ...
      {tasks.map((task, index) => (
        <div key={index} style={{ borderRadius: '5px', backgroundColor: 'pink', padding: '10px', marginBottom: '10px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', textDecoration: task.completed ? 'line-through' : 'none' }}>
          <input type="checkbox" checked={task.completed} onChange={() => handleTaskCompletionToggle(index)} style={{ marginRight: '5px' }} />
          {task.text}
          <FaTrash
